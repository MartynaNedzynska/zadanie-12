...
`;g(document.querySelector("#counter"));const p="https://kipkplviqflklgdjoxyg.supabase.co",s="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtpcGtwbHZpcWZsa2xnZGpveHlnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk0MTQ1MTAsImV4cCI6MjA2NDk5MDUxMH0.MS-azhCqRSWtDNkgnMYAv8KANtOxmLJS-SSQXFPr6r4",u={apikey:s,Authorization:`Bearer ${s}`,"Content-Type":"application/json"};async function a(){var e;const c=((e=document.getElementById("sortOrder"))==null?void 0:e.value)||"created_at.asc",r=await(await fetch(`${p}/rest/v1/articles?select=*&order=${c}`,{headers:u})).json(),i=document.getElementById("articles");i.innerHTML="",r.forEach(t=>{const n=document.createElement("div"),f=new Date(t.created_at).toLocaleDateString("pl-PL");n.innerHTML=`
      <h3>${t.title}</h3>
      <p><i>${t.subtitle}</i></p>
      ...
      <p>${t.content}</p>
      <hr>
    `,i.appendChild(n)})}var l;(l=document.getElementById("articleForm"))==null||l.addEventListener("submit",async c=>{c.preventDefault();const o=c.target,r={title:o.title.value,subtitle:o.subtitle.value,author:o.author.value,content:o.content.value,created_at:o.created_at.value};await fetch(`${p}/rest/v1/articles`,{method:"POST",headers:u,body:JSON.stringify(r)}),o.reset(),a()});var d;(d=document.getElementById("sortOrder"))==null||d.addEventListener("change",a);a();
