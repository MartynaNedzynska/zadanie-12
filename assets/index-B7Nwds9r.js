...
`;g(document.querySelector("#counter"));const p="https://kipkplviqflklgdjoxyg.supabase.co",s="twój_klucz_supabase_tutaj",u={apikey:s,Authorization:`Bearer ${s}`,"Content-Type":"application/json"};async function n(){var e;const c=((e=document.getElementById("sortOrder"))==null?void 0:e.value)||"created_at.asc",r=await(await fetch(`${p}/rest/v1/articles?select=*&order=${c}`,{headers:u})).json(),i=document.getElementById("articles");i.innerHTML="",r.forEach(t=>{const a=document.createElement("div"),f=new Date(t.created_at).toLocaleDateString("pl-PL");a.innerHTML=`
      <h3>${t.title}</h3>
      <p><i>${t.subtitle}</i></p>
      ...
      <p>${t.content}</p>
      <hr>
    `,i.appendChild(a)})}var l;(l=document.getElementById("articleForm"))==null||l.addEventListener("submit",async c=>{c.preventDefault();const o=c.target,r={title:o.title.value,subtitle:o.subtitle.value,author:o.author.value,content:o.content.value,created_at:o.created_at.value};await fetch(`${p}/rest/v1/articles`,{method:"POST",headers:u,body:JSON.stringify(r)}),o.reset(),n()});var d;(d=document.getElementById("sortOrder"))==null||d.addEventListener("change",n);n();
